...
import { creditsEasterEgg } from './credits-easter-egg';
  ...
  return `<dialog class="credits-dialog" aria-labelledby="credits-title" aria-describedby="credits-summary">
    ...
    <div class="credits-scroll" id="credits-scroll" tabindex="0" role="region" aria-label="플레이 감사 인사와 개발 중 안내">
      <div class="credits-roll">
        ...
        <figure id="credits-easter-egg" class="credits-easter-egg" hidden><video id="credits-easter-video" width="245" height="320" muted playsinline loop preload="none" aria-label="Good afternoon. 인사 장면"></video><figcaption><span>Good afternoon.</span><button id="credits-easter-motion" type="button" aria-controls="credits-easter-video" aria-pressed="false">재생</button></figcaption></figure>
      </div>
    </div>
...
export function startCredits(dialog: HTMLDialogElement, reducedMotion: MediaQueryList): () => void {
  ...
  const easterEgg = creditsEasterEgg(viewport, reducedMotion);
  ...
  function play(delay = 0): void {
    if (atEnd()) { easterEgg.reset(); viewport.scrollTop = 0; }
  ...
  viewport.addEventListener('scroll', () => {
    if (atEnd()) { pause(); easterEgg.reveal(); }
