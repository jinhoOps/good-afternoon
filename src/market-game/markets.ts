...
const walk: MarketDay[] = [
  parkDay(0, {
    title: '돗자리를 펴는 오후', weather: 'sun', weatherLabel: '가족 나들이', cost: 500,
    forecast: '손님 9~12명', forecastDetail: '아이와 산책 나온 가족이 많아요.',
    news: '“여럿이 마시려니 한 잔에 800원 정도면 좋겠어요.”',
    hint: '새로운 장터예요. 첫날부터 가격을 고를 수 있어요. 가볍게 사 마실 가족들이 기다려요.',
    visitors: guests([800, 900, 800, 1000, 900, 800, 1200, 800, 900, 1000, 800, 900], '가족과 나들이 왔어요.')
  }),
  parkDay(1, {
    title: '자전거를 세운 사람들', weather: 'heat', weatherLabel: '뜨거운 자전거 길', cost: 600,
    forecast: '손님 6~8명', forecastDetail: '긴 코스를 마친 자전거 모임이 쉬어가요.',
    news: '“멀리 달렸더니 목말라요. 1,500원까지는 괜찮아요.”',
    hint: '사람은 어제보다 적지만 시원한 음료를 찾고 있어요. 가격과 물량을 함께 바꿔볼까요?',
    visitors: guests([1500, 1600, 1500, 1200, 1700, 1500, 1300, 1500], '긴 코스를 달렸어요.')
  }),
  parkDay(2, {
    title: '비 소식과 남은 레몬', weather: 'rain', weatherLabel: '잠깐의 소나기', cost: 800,
    forecast: '손님 3~5명', forecastDetail: '정자에서 비를 피하는 산책객이 보여요.',
    news: '재료상 소식 · 오늘은 800원, 내일 새 물량이 오면 400원이에요.',
    hint: '내일 재료값이 내려가요. 보관료까지 내고 오늘 미리 만들 이유가 있을까요?',
    visitors: guests([1000, 1200, 1500, 1200], '비가 곧 그치겠죠?')
  }),
  parkDay(3, {
    title: '방과 후의 공원', weather: 'supply', weatherLabel: '레몬이 도착했어요', cost: 400,
    forecast: '손님 11~14명', forecastDetail: '학교가 끝나고 학생들이 모여들어요.',
    news: '“오늘 용돈은 넉넉하지 않아요. 천 원이면 한 잔 마실래요.”',
    hint: '재료값은 내려갔고, 주머니가 가벼운 손님은 늘었어요. 내일도 재료비는 같아요.',
    visitors: guests([800, 900, 1000, 1000, 1100, 1000, 1200, 1000, 900, 1000, 1100, 1200, 1000, 1000], '친구들과 잠깐 쉬러 왔어요.')
  }),
  parkDay(4, {
    title: '저녁 산책의 작은 선물', weather: 'festival', weatherLabel: '산책 주간의 마지막 날', cost: 400,
    forecast: '손님 9~12명', forecastDetail: '퇴근 후 산책 나온 이웃들이 들러요.',
    news: '“한 잔에 1,200원쯤이면 오늘의 작은 선물로 좋겠네요.”',
    hint: '비싼 가격을 택하면 놓치는 손님이 있어요. 마지막 진열대에 얼마나 놓을까요?',
    visitors: guests([1000, 1200, 1200, 1300, 1200, 1500, 1200, 1400, 1200, 1000, 1500, 1200], '하루를 마치고 걷고 있어요.')
  })
];

const concert: MarketDay[] = [
  parkDay(0, {
    title: '공연 준비가 한창이에요', weather: 'sun', weatherLabel: '무대 설치하는 날', cost: 600,
    forecast: '손님 5~7명', forecastDetail: '일을 마친 무대 스태프가 찾아와요.',
    news: '“쉬는 시간이 짧아요. 시원하면 1,500원도 괜찮아요.”',
    hint: '공연 주간이에요. 오늘은 가족 나들이 대신 스태프들이 잠깐 쉬어가요.',
    visitors: guests([1500, 1600, 1200, 1500, 1700, 1500], '무대를 준비하고 있어요.')
  }),
  parkDay(1, {
    title: '어린이 공연이 열려요', weather: 'festival', weatherLabel: '가족 관객의 날', cost: 500,
    forecast: '손님 12~15명', forecastDetail: '아이와 공연을 보러 온 가족이 많아요.',
    news: '“아이들과 나눠 마시려고요. 800원이면 부담 없겠어요.”',
    hint: '사람이 많아도 비싼 음료를 찾는 건 아니에요. 어제의 가격표를 그대로 둘까요?',
    visitors: guests([800, 900, 800, 1000, 800, 900, 800, 1200, 800, 900, 800, 1000, 800, 900, 800], '어린이 공연을 보러 왔어요.')
  }),
  parkDay(2, {
    title: '내일은 레몬이 귀해져요', weather: 'supply', weatherLabel: '공연 사이 쉬어가는 날', cost: 400,
    forecast: '손님 4~6명', forecastDetail: '산책 나온 이웃 몇 명이 들러요.',
    news: '재료상 소식 · 내일은 납품이 줄어 재료비가 400원 → 900원이에요.',
    hint: '오늘은 한산하지만 내일 재료값이 올라요. 다음 영업의 음료를 남겨둘 수도 있어요.',
    visitors: guests([1000, 1200, 1000, 1200, 900], '다음 공연은 내일이래요.')
  }),
  parkDay(3, {
    title: '재즈가 흐르는 해 질 녘', weather: 'heat', weatherLabel: '재즈 공연 전', cost: 900,
    forecast: '손님 8~10명', forecastDetail: '공연 전 여유를 즐기는 관객이 찾아와요.',
    news: '“공연 전에 한 잔 마실래요. 1,500원까지 생각하고 왔어요.”',
    hint: '재료가 비싼 날이에요. 내일 재료비는 600원으로 내려가요. 오늘 팔 만큼과 내일 물량을 구분해봐요.',
    visitors: guests([1500, 1700, 1500, 1200, 1600, 1500, 1800, 1500, 1300, 1500], '재즈 공연을 기다려요.')
  }),
  parkDay(4, {
    title: '잔디밭의 마지막 앙코르', weather: 'festival', weatherLabel: '모두 함께 앙코르', cost: 600,
    forecast: '손님 13~16명', forecastDetail: '산책객과 관객이 함께 마지막 공연을 봐요.',
    news: '“천 원이면 좋고, 1,200원까지는 한 잔 할래요.”',
    hint: '관객이 바뀌었어요. 어제의 높은 가격과 오늘의 많은 손님 사이를 생각해보세요.',
    visitors: guests([1000, 1200, 1200, 1300, 1000, 1200, 1500, 1200, 1000, 1200, 1300, 1500, 1000, 1200, 1200, 1300], '마지막 공연도 함께해요.')
  })
];
...
export const MARKETS: Record<MarketId, Market> = {
  ...
  'park-concert': { name: '공원 장터', edition: '공연 주간', description: '차를 찾는 손님은 절반쯤이고, 넷째 날은 더워요. 레몬값은 셋째 날 400원에서 다음 날 900원으로 올라요.', days: concert }
};
