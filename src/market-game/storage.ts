...
export function serialize(state: GameState): string {
  return JSON.stringify({ version: 3, market: state.market, equipment: state.equipment, records: state.records, firstShop: state.firstShop, phase: state.phase, order: state.order, receipts: state.receipts.map(({ quantity, price, cooler, booking }) => ({ quantity, price, cooler, booking })) });
}

export function restore(raw: string): GameState | null {
  try {
    const parsed: unknown = JSON.parse(raw);
    if (!record(parsed) || ![1, 2, 3].includes(parsed.version as number) || !Array.isArray(parsed.receipts) || parsed.receipts.length > DAYS.length) return null;
    ...
    if (parsed.version === 3) {
      ...
      for (const item of parsed.records) {
        ...
        const completed = restore(JSON.stringify({ version: 3, market: item.market, equipment: item.equipment, records: [], firstShop: null, phase: 'complete', receipts: item.orders }));
        if (!completed) return null;
        records.push(completed.records[0]);
      }
    }
    if (parsed.version === 3 && parsed.firstShop !== null) {
    ...
    let state = newGame(parsed.version === 3 ? parsed.market as GameState['market'] : 'neighborhood', records, parsed.version === 3 ? parsed.equipment as GameState['equipment'] : 'basic', firstShop);
    for (const [index, item] of parsed.receipts.entries()) {
      ...
      if (parsed.version === 3 && typeof item.booking !== 'boolean') return null;
      if (index > 0) state = nextDay(state);
      const ordered = changeOrder(state, { quantity: item.quantity, price: item.price, cooler: parsed.version !== 1 ? item.cooler as boolean : false, booking: parsed.version === 3 ? item.booking as boolean : false });
    ...
    if (parsed.phase === 'complete' && state.receipts.length === DAYS.length) state = nextDay(state);
      ...
      if (parsed.version === 3 && typeof parsed.order.booking !== 'boolean') return null;
      const updated = changeOrder(state, { quantity: parsed.order.quantity, price: parsed.order.price, cooler: parsed.version !== 1 ? parsed.order.cooler as boolean : false, booking: parsed.version === 3 ? parsed.order.booking as boolean : false });
