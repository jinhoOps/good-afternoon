...
function planning(state: GameState): string {
  ...
  return `<section class="play-panel" aria-labelledby="panel-title">
    ...
    ${state.equipment === 'tea' ? `<p class="menu-notice"><strong>찻잎 재료비 +600원이 포함돼요.</strong>${teaInterest(state).hint}<small>소식의 손님 반응은 레모네이드 기준이에요. 만든 차는 식혀 보관하고 다음 날 데워 팔아요.</small></p>` : ''}${stockNotice(state)}${bookingDue(state)}
    ...
    <fieldset class="price-block"><legend>한 잔 판매가 ${fixedPrice(state) ? '<span>첫날은 3,000원으로 시작해요</span>' : ''}</legend>${fixedPrice(state) ? `<div class="fixed-price"><strong>3,000<span>원</span></strong><span>가격 정하기는 다음 오후부터</span></div>` : `<div class="price-options">${PRICES.map((price) => `<button type="button" id="price-${price}" data-action="price" data-value="${price}" aria-pressed="${state.order.price === price}">${price.toLocaleString('ko-KR')}<small>원</small></button>`).join('')}</div>`}</fieldset>
