import { hasShop, unitCost, rentalFee, storageLimit, drinkName, teaInterest, type Equipment } from './equipment';
import { dayFor, daysFor, fixedPrice, MARKETS } from './markets';
import { marketChoices, journeyRecords, shopScene } from './journey-view';
import { DAYS, DREAM_CASH, INITIAL_CASH, PRICES, money } from './content';
import { preparationCapacity, minimumPreparation, resultNote, type GameState, type Receipt } from './domain';
import { bookingChoice, bookingDue, bookingResult, bookingCashRows } from './contracts-view';
...
function liveReceipt(state: GameState, ui: ViewState): { sold: number; cash: number; left: number } {
  ...
  const walkInSold = last.outcomes.slice(0, ui.seen).filter((outcome) => outcome.kind === 'bought').length;
  const sold = walkInSold + (last.delivered?.quantity ?? 0);
  return { sold, cash: last.openingCash - last.cost - last.coolerCost + walkInSold * last.price, left: last.totalStock - sold };
}
...
function scene(state: GameState, ui: ViewState): string {
  ...
  return `<section class="world-column" aria-label="오후의 레모네이드 가게">
    <div class="scene ${state.equipment === 'tea' ? 'tea-scene' : ''} weather-${day.weather} ${ui.selling ? 'is-selling' : ''}">
      ...
      ${shopScene(state)}${coolerScene(state, ui.selling, live.sold)}
      <div class="counter-display"><div class="counter-caption">${icon('cup')} ${showResult ? '남은 음료' : ui.selling ? '지금 진열대' : `준비할 ${drinkName(state)}`}<strong>${cups}<span>잔</span></strong></div><div class="cup-row" aria-hidden="true">${Array.from({ length: Math.min(12, cups) }, () => icon('cup', 'stock-cup')).join('')}${cups > 12 ? `<span class="more-cups">+${cups - 12}</span>` : ''}${cups === 0 ? '<span class="empty-shelf">비어 있는 진열대</span>' : ''}</div></div>
    </div>
    <div class="market-news"><span class="news-icon">${icon('people')}</span><div><div class="news-topline"><strong>${day.forecast}</strong><span>오늘의 장터 소식</span></div><p class="forecast-detail">${day.forecastDetail}</p><p>${day.news}</p></div></div>
    <div class="dream"><span class="dream-icon">${icon('leaf')}</span><div class="dream-copy"><strong>${hasShop(state) ? '우리 가게의 간판이 생겼어요' : '언젠가, 내 이름을 건 가게'}</strong><p>${hasShop(state) ? '간판과 설비 선택은 다음 장터에도 가져가요.' : `다섯 번의 오후 동안 준비금 ${money(DREAM_CASH)} 모아보기`}</p></div>${hasShop(state) ? '' : `<div class="dream-progress" role="progressbar" aria-label="가게 준비금" aria-valuemin="0" aria-valuemax="${DREAM_CASH}" aria-valuenow="${Math.min(DREAM_CASH, live.cash)}"><span style="width:${Math.min(100, live.cash / DREAM_CASH * 100)}%"></span></div>`}</div>
  </section>`;
}
...
function planning(state: GameState): string {
  ...
  const min = minimumPreparation(state);
  const cost = state.order.quantity * unitCost(state);
  const coolerCost = state.order.cooler ? rentalFee(state) : 0;
  return `<section class="play-panel" aria-labelledby="panel-title">
    <div class="panel-eyebrow">${icon('cup')} 오늘의 메뉴 · ${drinkName(state)}</div>
    <h2 id="panel-title" tabindex="-1">오늘은 몇 잔<br> 준비해볼까요?</h2>
    <p class="panel-intro">${day.hint}</p>
    ${state.equipment === 'tea' ? `<p class="menu-notice"><strong>찻잎 재료비 +200원이 포함돼요.</strong>${teaInterest(state).hint}<small>소식의 손님 반응은 레모네이드 기준이에요. 만든 차는 식혀 보관하고 다음 날 데워 팔아요.</small></p>` : ''}${stockNotice(state)}${bookingDue(state)}
    <div class="order-block"><div class="field-heading"><label for="quantity">${state.stock.quantity ? '새로 만들 음료' : '준비할 음료'}</label><span>한 잔 재료비 ${money(unitCost(state))}</span></div>
      <div class="stepper"><button id="less" data-action="less" aria-label="한 잔 줄이기" ${state.order.quantity === min ? 'disabled' : ''}>−</button><div class="quantity-display"><input id="quantity" aria-label="준비할 음료 수량" type="number" inputmode="numeric" min="${min}" max="${max}" value="${state.order.quantity}"><span>잔</span></div><button id="more" data-action="more" aria-label="한 잔 늘리기" ${state.order.quantity === max ? 'disabled' : ''}>+</button></div>
      <div class="quantity-presets" aria-label="준비 수량 빠르게 선택">${[4, 8, 12].map((qty) => `<button id="preset-${qty}" data-action="quantity" data-value="${qty}" class="${state.order.quantity === qty ? 'selected' : ''}" ${qty > max || qty < min ? 'disabled' : ''}>${qty}잔</button>`).join('')}</div>
    </div>
    <fieldset class="price-block"><legend>한 잔 판매가 ${fixedPrice(state) ? '<span>첫날은 1,000원으로 시작해요</span>' : ''}</legend>${fixedPrice(state) ? `<div class="fixed-price"><strong>1,000<span>원</span></strong><span>가격 정하기는 다음 오후부터</span></div>` : `<div class="price-options">${PRICES.map((price) => `<button type="button" id="price-${price}" data-action="price" data-value="${price}" aria-pressed="${state.order.price === price}">${price.toLocaleString('ko-KR')}<small>원</small></button>`).join('')}</div>`}</fieldset>
    ${coolerChoice(state)}${bookingChoice(state)}
    ...
    <p class="perish-note">${icon('info')} ${state.day < 2 ? '남은 음료는 다음 날 팔 수 없어요.' : state.order.cooler ? `오늘 만든 음료만 최대 ${storageLimit(state)}잔, 내일까지만 보관해요.` : state.day === DAYS.length - 1 ? '마지막 영업이에요. 남은 음료는 정리해요.' : '보관함 없이 남은 음료는 다음 날 팔 수 없어요.'}</p>
...
function selling(state: GameState, ui: ViewState): string {
  ...
  const live = liveReceipt(state, ui);
  return `<section class="play-panel selling-panel" aria-labelledby="panel-title"><div class="panel-eyebrow"><span class="live-dot active"></span> 오늘의 영업</div><h2 id="panel-title" tabindex="-1">어서 오세요,<br> ${state.equipment === 'tea' ? '따뜻한 차 한 잔' : '시원한 한 잔'} 어때요?</h2><p class="panel-intro">손님들의 반응을 살펴보세요.<br> 준비한 물량과 가격이 어떻게 돌아올까요?</p>${receipt.delivered ? `<p class="booking-due">단체 주문 ${receipt.delivered.quantity}잔은 먼저 전달했어요.<br>대금은 어제 받았고, 이제 일반 손님을 만나요.</p>` : ''}<div class="live-sales"><div class="large-cup">${icon('cup')}</div><strong>${live.sold}<span>잔 팔았어요</span></strong><p>지금까지 받은 돈 <b>${money((live.sold - (receipt.delivered?.quantity ?? 0)) * receipt.price)}</b></p></div><div class="visitor-track" aria-label="손님 ${receipt.outcomes.length}명 중 ${ui.seen}명 방문">${receipt.outcomes.map((outcome, index) => `<span class="${index < ui.seen ? outcome.kind : ''}" aria-hidden="true"></span>`).join('')}</div><p class="visitors-progress">${ui.seen} / ${receipt.outcomes.length}명 방문</p><button id="skip-sales" class="secondary-button" data-action="skip">영업 결과 바로 보기 ${icon('arrow')}</button></section>`;
}

function receiptRows(receipt: Receipt): string {
  return `<div class="receipt-rows"><div><span>일반 손님에게 받은 돈 <small>${receipt.walkInSold}잔 × ${money(receipt.price)}</small></span><strong>+${money(receipt.walkInSold * receipt.price)}</strong></div>${bookingCashRows(receipt)}<div><span>재료에 쓴 돈 <small>오늘 만든 ${receipt.quantity}잔</small></span><strong>−${money(receipt.cost)}</strong></div>${receipt.coolerCost ? `<div><span>보관함 대여료</span><strong>−${money(receipt.coolerCost)}</strong></div>` : ''}<div class="profit-row"><span>오늘 현금 변화</span><strong class="${receipt.cashChange < 0 ? 'negative' : ''}">${difference(receipt.cashChange)}</strong></div></div>`;
}

function result(state: GameState): string {
  const day = dayFor(state);
  const receipt = state.receipts.at(-1)!;
  return `<section class="play-panel result-panel" aria-labelledby="panel-title"><div class="panel-eyebrow">${icon('check')} ${day.label}의 기록</div><h2 id="panel-title" tabindex="-1">${receipt.leftover > 0 ? '오늘의 경험도<br> 가게에 남았어요.' : receipt.totalStock === 0 ? '잠시 쉬어가는<br> 오후도 있어요.' : '한 잔씩, 차곡차곡.<br> 수고했어요!'}</h2><div class="sales-summary"><span><strong>${receipt.sold}</strong>잔 판매</span><span><strong>${receipt.leftover}</strong>잔 남음</span><span><strong>${receipt.expensive + receipt.missed}</strong>명 돌아감</span></div>${receiptRows(receipt)}${storageResult(receipt)}${bookingResult(receipt)}<p class="result-observation">${resultNote(receipt)}</p>${inventoryAccounting(receipt)}<details class="concept-note"><summary>${icon('leaf')} 오늘 만난 경제 · ${day.concept.title}<span>+</span></summary><p>${day.concept.description}</p><small>${state.day < 2 ? '이 작은 가게에서는 재료비만 계산해요.' : '이 작은 가게에서는 재료비와 보관함 대여료만 계산해요.'} 실제 장사에는 임대료 등 다른 비용도 있어요.</small></details><button id="next-day" class="primary-button" data-action="next">${state.day === DAYS.length - 1 ? '다섯 번의 오후 돌아보기' : '다음 오후로'} ${icon('arrow')}</button><button id="retry-day" class="text-button retry" data-action="retry">${icon('repeat')} 다른 선택으로 오늘 다시 해보기</button><p class="retry-note">다시 하면 오늘 영업 전의 돈으로 돌아가요.</p></section>`;
}
...
function modal(state: GameState, kind: ViewState['modal'], equipment: Equipment = state.equipment): string {
  ...
  if (kind === 'journal') {
    title = '내 장터 일지';
    content = state.receipts.length ? state.receipts.map((receipt) => `<article class="journal-entry"><h3>${receipt.day + 1}일차 · ${daysFor(state)[receipt.day].title}</h3><p>새 음료 ${receipt.quantity}잔${receipt.openingStock.quantity ? ` + 보관분 ${receipt.openingStock.quantity}잔` : ''} · 판매가 ${money(receipt.price)} · ${receipt.sold}잔 판매</p>${receiptRows(receipt)}${storageResult(receipt)}${bookingResult(receipt)}${inventoryAccounting(receipt)}<p>${resultNote(receipt)}</p></article>`).join('') : '<div class="empty-journal">첫 영업이 끝나면<br> 이곳에 오늘의 기록이 남아요.</div>';
  }
  if (kind === 'journal') content = journeyRecords(state) + content;
  if (kind === 'markets') {
    title = '다음엔 어디에 가게를 열까요?';
    content = marketChoices(state, equipment);
  }
  if (kind === 'reset') {
    title = '새 장터를 열까요?';
    content = '<p class="reset-copy">이번 영업을 6,000원으로 첫 오후부터 다시 시작해요. 장터별 최고 기록은 남아요.</p><button id="confirm-reset" class="primary-button" data-action="confirm-reset">새 장터 시작하기</button><button class="secondary-button" data-action="close-modal">현재 장터 계속하기</button>';
  }
