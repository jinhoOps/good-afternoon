...
export function marketChoices(state: GameState, equipment: Equipment): string {
  ...
  return `<p class="journey-intro">운영 자금은 각각 18,000원으로 시작해요. 장터별 최고 기록은 이 브라우저에 남아요.</p><p class="journey-preview"><strong>다음 공원 소식 · ${MARKETS[park].edition}</strong>${MARKETS[park].description}</p>${equipmentChoices(state, equipment)}
...
function equipmentChoices(state: GameState, equipment: Equipment): string {
  if (!hasShop(state)) return '<p class="journey-note">어느 장터에서든 마지막에 준비금 45,000원이 모이면 우리 가게 간판과 설비 자리 하나가 생겨요.</p>';
  const options = [
    ['basic', '익숙한 진열대', '레모네이드 · 기본 보관 4잔, 대여 하루 1,800원'],
    ['pantry', '넓은 보관 자리', '레모네이드 · 보관 8잔, 대여 하루 3,000원'],
    ['tea', '새 메뉴 자리', '따뜻한 레몬티 · 재료비 +600원, 손님 취향에 따라 구매 반응 변화']
  ];
