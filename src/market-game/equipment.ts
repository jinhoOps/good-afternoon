...
export const unitCost = (state: ShopState): number => dayFor(state).cost + (state.equipment === 'tea' ? 200 : 0);
export const rentalFee = (state: { equipment: Equipment }): number => state.equipment === 'pantry' ? 1000 : COOLER_FEE;
...
export function teaInterest(state: Pick<ShopState, 'market' | 'day'>): { change: number; mixed: boolean; hint: string } {
  ...
  if (weather === 'rain') return { change: 500, mixed: false, hint: '비 오는 날, 따뜻한 차라면 500원쯤 더 내겠다는 손님들이에요.' };
  if (weather === 'heat') return { change: -300, mixed: false, hint: '더운 날이라 따뜻한 차에는 시원한 음료보다 300원쯤 덜 쓰고 싶어 해요.' };
  if (state.market === 'park-walk') return { change: 600, mixed: false, hint: '오늘 산책객들은 따뜻한 차를 찾고 있어요. 레모네이드보다 600원 더 쓸 생각이에요.' };
  return { change: 300, mixed: true, hint: '절반쯤은 차를 좋아해 300원 더, 나머지는 시원한 음료를 원해 200원 덜 쓰려고 해요.' };
}
...
export function willingness(state: ShopState, index: number): number {
  ...
  const adjustment = !interest.mixed || index % 2 === 0 ? interest.change : -200;
