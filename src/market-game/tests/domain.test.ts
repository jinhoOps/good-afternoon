...
test('10잔을 준비하고 6잔을 팔면 매출은 18,000원, 이익은 3,000원입니다', () => {
  const state = openMarket(changeOrder(newGame(), { quantity: 10, price: 3000 }));
  const receipt = state.receipts[0];
  assert.equal(receipt.sold, 6);
  assert.equal(receipt.leftover, 4);
  assert.equal(receipt.revenue, 18000);
  assert.equal(receipt.cost, 15000);
  assert.equal(receipt.profit, 3000);
  assert.equal(state.cash, 21000);
});

test('준비한 재고보다 팔지 않고, 재료 예산을 초과하지 않습니다', () => {
  const initial = newGame();
  for (const quantity of [-1, 13, NaN, Infinity, 1.5]) {
    assert.equal(changeOrder(initial, { quantity, price: 3000 }), initial);
  }
  assert.equal(changeOrder(initial, { quantity: 6, price: 4500 }), initial);
  const state = openMarket(changeOrder(initial, { quantity: 4, price: 3000 }));
...
test('가격과 손님 조건에 따라 판매량이 달라집니다', () => {
  ...
  const cheap = openMarket(changeOrder(dayTwo, { quantity: 12, price: 2400 }));
  const premium = openMarket(changeOrder(dayTwo, { quantity: 12, price: 4500 }));
...
test('연타와 결과 상태의 조작으로 정산이 중복되지 않습니다', () => {
  ...
  assert.equal(changeOrder(result, { quantity: 10, price: 3000 }), result);
...
test('다시 해보기는 정확한 영업 전 돈과 이전 기록으로 돌아갑니다', () => {
  ...
  const result = openMarket(changeOrder(dayTwo, { quantity: 8, price: 4500 }));
  ...
  assert.equal(retried.order.price, 4500);
});
  ...
  for (const price of PRICES) {
    for (const requested of [0, 2, 6, 12, 18]) {
      ...
      for (let day = 0; day < DAYS.length; day++) {
        state = changeOrder(state, { quantity: Math.min(requested, capacity(state.cash, day)), price: day === 0 ? 3000 : price });
...
test('저장값의 잔액을 믿지 않고 선택 기록에서 다시 계산합니다', () => {
  ...
  payload.cash = 300000000;
  payload.receipts[0].profit = 270000000;
