...
test('v2의 대여와 보관을 모든 단계에서 잃지 않고 이전합니다', () => {
  ...
  for (let day = 0; day < 5; day++) {
    const cooler = day === 2;
    state = changeOrder(state, { quantity: Math.min(day === 2 ? 9 : 6, preparationCapacity(state, cooler)), price: day === 2 ? 800 : 1000, cooler });
    for (const phase of ['planning', 'result'] as const) {
      if (phase === 'result') state = openMarket(state);
      const legacy = { version: 2, phase, order: { quantity: state.order.quantity, price: state.order.price, cooler: state.order.cooler }, receipts: state.receipts.map(({ quantity, price, cooler }) => ({ quantity, price, cooler })) };
      assert.deepEqual(restore(JSON.stringify(legacy)), state);
    }
    state = nextDay(state);
  }
  const legacy = { version: 2, phase: 'complete', receipts: state.receipts.map(({ quantity, price, cooler }) => ({ quantity, price, cooler })) };
  assert.deepEqual(restore(JSON.stringify(legacy)), state);
});
...
test('설비는 보관 용량과 비용, 메뉴 원가와 손님 취향을 함께 바꿉니다', () => {
  ...
  assert.equal(rentalFee(pantry) - rentalFee(basic), 400);
  assert.equal(unitCost(tea) - unitCost(basic), 200);
  assert.equal(willingness({ ...tea, day: 2 }, 0) - willingness({ ...basic, day: 2 }, 0), 500);
  assert.equal(willingness({ ...tea, day: 1 }, 0) - willingness({ ...basic, day: 1 }, 0), -300);
  assert.equal(willingness(tea, 0) - willingness(basic, 0), 600);
  assert.equal(willingness({ ...tea, market: 'neighborhood' }, 1) - willingness({ ...basic, market: 'neighborhood' }, 1), -200);
});
...
test('선불금은 받은 날의 이익이 아니며 다음 날 약속 물량과 준비 자금을 확보합니다', () => {
  const dayTwo = nextDay(openMarket(changeOrder(newGame('park-walk'), { quantity: 12, price: 800 })));
  const accepted = openMarket(changeOrder(dayTwo, { quantity: 6, price: 1500, booking: true }));
  const r = accepted.receipts.at(-1)!;
  assert.equal(accepted.cash, 23000);
  assert.equal(r.profit, 5400);
  assert.equal(r.cashChange - r.profit, 8000);
  ...
  assert.equal(changeOrder(tomorrow, { quantity: 7, price: 1200 }), tomorrow);
  assert.deepEqual(restore(serialize(tomorrow)), tomorrow);
  const result = openMarket(changeOrder(tomorrow, { quantity: 11, price: 1200 }));
  assert.equal(result.cash, 17800);
  const delivered = result.receipts.at(-1)!;
  assert.equal(delivered.walkInSold, 3);
  assert.equal(delivered.sold, 11);
  assert.equal(delivered.revenue, 11600);
  assert.equal(delivered.cashChange, -5200);
  assert.equal(delivered.profit, 2800);
...
test('장터·설비·가격·물량·보관·예약 조합에서 의무와 돈·재고를 보존합니다', () => {
  for (const market of MARKET_IDS) for (const equipment of EQUIPMENT_IDS) for (const price of PRICES) for (const quantity of [0, 6, 12, 18]) for (const rent of [false, true]) for (const book of [false, true]) {
    ...
    for (let day = 0; day < 5; day++) {
      ...
      state = changeOrder(state, { quantity: Math.max(minimumPreparation(state), Math.min(quantity, preparationCapacity(state, cooler, booking))), price: fixedPrice(state) ? 1000 : price, cooler, booking });
      const before = state;
